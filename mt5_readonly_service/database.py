...
class SimulatedPosition(Base):
    ...
    # Auto-generated shadow trades (created from watchlist signals that
    # triggered a Telegram alert -- tracked automatically by the system).
    auto_generated    = Column(Integer, nullable=False, default=0)  # 0/1 boolean
    source_signal_id  = Column(Integer, nullable=True)
    expected_close_at = Column(DateTime(timezone=True), nullable=True)
    close_reason      = Column(String(20), nullable=True)  # HIT_TP | HIT_SL | TIMEOUT | NULL (manual)
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "autoGenerated":       bool(self.auto_generated),
            "sourceSignalId":      self.source_signal_id,
            "expectedCloseAt":     self.expected_close_at.isoformat() if self.expected_close_at else None,
...
class MT5TradeHistory(Base):
    ...
    def to_dict(self) -> dict:
        is_crypto = any(x in self.symbol.upper() for x in ["BTC", "ETH", "USDT"])
        return {
            "id":                      self.id,
            ...
            "source":                  "okx" if is_crypto else "mt5",
        }
    ...
    if "simulated_positions" in inspector.get_table_names():
        ...
        with engine.begin() as conn:
            ...
            if "auto_generated" not in columns:
                conn.execute(text("ALTER TABLE simulated_positions ADD COLUMN auto_generated INTEGER NOT NULL DEFAULT 0"))
            if "source_signal_id" not in columns:
                conn.execute(text("ALTER TABLE simulated_positions ADD COLUMN source_signal_id INTEGER"))
            if "expected_close_at" not in columns:
                conn.execute(text("ALTER TABLE simulated_positions ADD COLUMN expected_close_at DATETIME"))
            if "close_reason" not in columns:
                conn.execute(text("ALTER TABLE simulated_positions ADD COLUMN close_reason VARCHAR(20)"))
